...
# RAG يتم تحميله فقط عند الحاجة حتى لا يبطئ أسئلة SQL
rag_module = None
...
RAG_KEYWORDS = [
    ...
    "خطورة", "خطر", "مخاطر", "تأثير", "اثر", "أثر", "آثار", "اضرار", "أضرار", "ضرر",
...
def is_pure_knowledge_question(question: str) -> bool:
    """سؤال معرفي بحت — بدون إشارات أرقام/قاعدة بيانات."""
    q = question.lower()
    knowledge_markers = [
        "ما الفرق", "الفرق بين", "ما هو", "ماهو", "ما هي", "ماهي", "تعريف",
        "difference between", "what is", "define", "explain",
    ]
    if not any(m in q for m in knowledge_markers):
        return False
    return not _contains_any(question, SQL_KEYWORDS)
...
def route_with_rules(question: str) -> str | None:
    ...
    """
    if is_pure_knowledge_question(question):
        return "rag"
...
- إذا كان السؤال معرفياً بحتاً (مثل: ما الفرق بين X و Y، ما هو، تعريف) فاعتمد على إجابة المستندات فقط ولا تذكر إحصائيات قاعدة البيانات إلا إذا كانت مرتبطة مباشرة بالسؤال.
- إذا كان السؤال عن بيانات/أرقام/حالات محددة: ابدأ بالحقائق من قاعدة البيانات ثم اربطها بالمستندات.
