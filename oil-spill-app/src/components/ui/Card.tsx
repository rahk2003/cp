...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-2xl border border-ocean-100 bg-white shadow-[0_1px_3px_rgba(11,30,63,0.04),0_8px_24px_-12px_rgba(11,30,63,0.08)]",
        className
      )}
      {...props}
    />
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn("text-lg font-semibold text-navy-500 tracking-tight", className)}
      {...props}
    />
  )
);
